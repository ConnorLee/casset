...
export function ArtistCarousel({ artists, onArtistSelect, selectedArtist }: ArtistCarouselProps) {
  ...
  const getStyles = useMemo(() => {
    ...
    return {
      getImageSize: (index: number) => {
        ...
        if (distance === 0) return "w-[160px] h-[160px] md:w-[200px] md:h-[200px]"
        if (distance === 1) return "w-20 h-20 md:w-28 md:h-28"
  ...
  return (
    <div
      ref={carouselRef}
      className="relative w-full max-w-5xl mx-auto py-8 md:py-16 flex items-center justify-center h-[300px] md:h-[350px]"
      role="region"
